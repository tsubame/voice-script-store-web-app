  ...
  <v-card :class="{
    ...
    <v-list-item class="my-0 px-0">      
      ...
      <p class="mt-2 pl-0 text-grey-darken-4 text-caption">
        {{ props.targetScript.minutes }}分　{{ props.targetScript.actorCount }}人　{{ props.targetScript.authorName }}
      </p>       
      <p class="mt-4 mb-0">
        <!-- カテゴリチップ -->
        <span
      ...
      <div id="fav-count-container" 
        :class="{
          'text-body-2': true,
          'mobile-fav-count-container': useDisplay().smAndDown.value,
          }">
        <template v-if="isLiked">
          <v-icon class="h-icon text-h6" color="amber-lighten-3">mdi-star</v-icon>                     
